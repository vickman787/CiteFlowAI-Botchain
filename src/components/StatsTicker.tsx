...
export default function StatsTicker({ stats }: { stats: NetworkStats }) {
  const items = [
    <span key="net" className="flex items-center gap-2">
      ...
      <span className="text-[var(--color-signal-green)]">BOT-CHAIN · LIVE</span>
    </span>,
    <span key="ans">ANSWERS SERVED: <b className="text-[var(--color-ink)]">{stats.answersServed.toLocaleString('en-US')}</b></span>,
    <span key="paid">PAID TO CREATORS: <b className="text-[var(--color-signal-green)]">${stats.paidToCreators.toFixed(2)} USDT</b></span>,
    ...
    <span key="w3s">CHAIN 677 · USDT SETTLEMENTS</span>,
  ]
