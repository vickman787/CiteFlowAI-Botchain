...
export async function resolveArcPost(targetUrl: string): Promise<ArcPostResult> {
  ...
  return {
    ...
    text,
    canonicalUrl: canonicalArcUrl(html, targetUrl),
  }
}

// The same post is served at both /home/forum/... (the in-app path members see
// and copy from the address bar) and /public/forum/... — same author, same body,
// different URL. Since a source row is keyed on its URL, leaving them distinct
// would let one post be registered twice and paid twice for a single citation.
// Every post page emits a canonical link pointing at the /public form, so use it.
function canonicalArcUrl(html: string, targetUrl: string): string {
  const declared = html.match(/<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']+)["']/i)?.[1]

  if (declared) {
    try {
      const parsed = new URL(declared, targetUrl)
      // Only trust it if it stays on Arc House — a canonical tag pointing
      // somewhere else must never redirect where a source gets registered.
      if (ARC_HOSTS.has(stripWww(parsed.hostname))) {
        return `${parsed.origin}${parsed.pathname}`
      }
    } catch {
      // fall through to the path rewrite below
    }
  }

  // No usable canonical tag: normalize the known alias by hand.
  const stripped = stripQuery(targetUrl)
  return stripped.replace('/home/forum/', '/public/forum/')
}
